...
df = pd.read_csv(os.path.join(sys.argv[1],'sample_sub.csv'))
batch_size = 100
pred = []
image2 = []
with open(os.path.join(sys.argv[1],'sample_sub.csv'), 'r') as file:
    csv_reader = csv.reader(file)
    i = 0
    for row in csv_reader:
        # print(i)
        ...
        i += 1
#         print(row[0])
        image_path = os.path.join(sys.argv[1], f"HandwrittenData/images/test/{row[0]}")
        image = Image.open(image_path).convert("RGB")
        preprocessed_image = transformer(image)
