...
csv_file_path2 = os.path.join(sys.argv[1], 'HandwrittenData/train_hw.csv')
vocab_init2 = []
images2 = []
...
latex_vocab, inv_vocab = create_vocab(vocab_init + vocab_init2)
batch_size = 64
...
latex_dataset2 = LatexDataset(vocab_init2, images2)
dataloader2 = DataLoader(latex_dataset2, batch_size=batch_size, shuffle=True)
...
for epoch in range(10):
    ...
    for batch in dataloader:
        ...
        loss_decoder = train_decoder(decoder, target_tensor, context_vector, criterion_decoder, optimizer_decoder,
                                     use_teacher_forcing)
    ...
    print(f'Epoch syn [{epoch + 1}/{num_epochs}], Decoder Loss: {average_loss_decoder:.4f}')
...
for epoch in range(5):
    ...
    # Print or log the average loss for the epoch
    average_loss_decoder = total_loss_decoder / len(dataloader)
    print(f'Epoch syn + han [{epoch + 1}/{num_epochs}], Decoder Loss: {average_loss_decoder:.4f}')
